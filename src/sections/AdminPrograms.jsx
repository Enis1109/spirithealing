    ...
    const uploadWeekAsset = async (weekNumber, kind, file) => {
        ...
        try {
            ...
            const uploadFields = {
                ...
                meditationcover: "meditationImage",
            ...
            const uploadLabels = {
                ...
                meditationcover: "Das Cover der Meditation",
                        ...
                        return (
                            <article key={week.weekNumber} className="rounded-3xl bg-[#fffaf2] p-5 shadow-sm sm:p-7">
                                ...
                                <div className="mt-6 grid gap-5 lg:grid-cols-2">
                                    ...
                                    <div className="font-bold lg:col-span-2">Cover der Meditation<input className={fieldClass} value={week.meditationImage || ""} onChange={(event) => setWeekField(week.weekNumber, "meditationImage", event.target.value)} /><label className="mt-2 inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-full border border-[#0f8b8d] px-4 text-sm text-[#0f8b8d]"><Upload className="h-4 w-4" />{busyKey === `asset-${week.weekNumber}-meditationcover` ? "Wird hochgeladen …" : "Cover geschützt hochladen"}<input type="file" className="sr-only" accept="image/jpeg,image/png,image/webp" disabled={Boolean(busyKey)} onChange={(event) => { uploadWeekAsset(week.weekNumber, "meditationcover", event.target.files?.[0]); event.target.value = ""; }} /></label></div>
